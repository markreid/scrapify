...
function ask(str) {
  ...
  return new Promise((res) =>
    readlineInterface.question(chalk.yellow(str), (answer) => {
      res(answer);
...
function scrapeSearchTerms(page, selectors) {
  ...
  const allResults = selectors.map((selector) =>
    Array.from(dom.window.document.querySelectorAll(selector)).map((element) =>
